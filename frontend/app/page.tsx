...
// Dégradé fixe utilisé uniquement pour les bandeaux photo (hero/CTA) : ces
// sections gardent un fond sombre volontaire quel que soit le thème du site.
const NAVY = '#185FA5';
...
function VehicleCard({ v }: { v: Voiture }) {
  ...
  return (
    <Link href={`/catalogue/${v.id}`}
      className="group flex flex-col overflow-hidden rounded-2xl bg-[var(--color-background-primary)] shadow-md ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:ring-[var(--color-accent-ring)] w-72 shrink-0 snap-start">
      <div className="relative h-52 overflow-hidden bg-[var(--color-accent-light)]">
        {photos.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center gap-2">
            <svg className="h-14 w-14 text-[var(--color-text-secondary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span className="text-xs text-[var(--color-text-secondary)]">Photo à venir</span>
          </div>
        ) : photos.map((src, i) => (
        ...
        <div className="absolute left-3 top-3 z-10 flex gap-1.5">
          {isLoc && <span className="rounded-full bg-[var(--color-accent)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white backdrop-blur-sm">Location</span>}
        ...
        {v.energie && (
          <span className="absolute right-3 top-3 z-10 rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-bold text-[var(--color-accent)] shadow-sm backdrop-blur-sm">
            {v.energie}
          </span>
        )}
        {multi && (
        ...
        <div className="absolute bottom-0 inset-x-0 z-10 px-4 pb-4 pt-8">
          <div className="flex items-end justify-between">
            ...
            <span className="rounded-xl bg-white/15 px-3 py-1.5 text-xs font-bold text-white backdrop-blur-sm ring-1 ring-white/25 transition group-hover:bg-white group-hover:text-[var(--color-accent)]">
              Voir →
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-2 p-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-secondary)]">{v.marque}</p>
          <p className="mt-0.5 text-base font-black text-[var(--color-text-primary)]">
            {v.modele}{v.annee && <span className="ml-1.5 text-sm font-normal text-[var(--color-text-secondary)]">{v.annee}</span>}
          </p>
        </div>
...
const HOW_IT_WORKS = [
  ...
  },
  {
    ...
    image: '/client-remise-cles-femme.jpg',
  },
];
...
export default function HomePage() {
  ...
  return (
    <PublicLayout>
            ...
            <div className="max-w-2xl">
              ...
              <form onSubmit={handleSearch}
                ...
                <select
                  ...
                  className="flex-1 min-w-[140px] rounded-xl bg-white/95 px-4 py-3 text-sm font-semibold text-[var(--color-accent)] outline-none focus:ring-2 focus:ring-white/60">
                ...
                <select
                  ...
                  className="flex-1 min-w-[120px] rounded-xl bg-white/95 px-4 py-3 text-sm font-semibold text-[var(--color-accent)] outline-none focus:ring-2 focus:ring-white/60">
                  <option value="">Tous types</option>
                  <option value="location">Location</option>
                  <option value="vente">Vente</option>
                </select>
                <button type="submit"
                  className="rounded-full px-6 py-3 text-sm font-black text-white shadow-md transition hover:opacity-90 flex items-center gap-2 whitespace-nowrap"
      ...
      {/* ── STATS ────────────────────────────────────────────────────────────── */}
      <section className="bg-[var(--color-background-primary)] py-6 border-b border-[var(--color-border-tertiary)] overflow-hidden">
        <div className="flex animate-marquee">
          {[...STATS, ...STATS].map((s, i) => (
            <div key={i} className="flex shrink-0 items-center gap-3 px-10">
              <p className="text-2xl font-black whitespace-nowrap" style={{ color: 'var(--color-accent)' }}>{s.value}</p>
              <p className="text-sm font-medium text-[var(--color-text-secondary)] whitespace-nowrap">{s.label}</p>
              <span className="ml-4 h-6 w-px bg-[var(--color-border-tertiary)]" />
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── MARQUES ──────────────────────────────────────────────────────────── */}
      <section className="border-b border-[var(--color-border-tertiary)] bg-[var(--color-background-secondary)] py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal flex flex-wrap items-center justify-center gap-x-8 gap-y-4">
            {BRANDS.map((brand) => (
              <span key={brand}
                className="text-base font-black tracking-tight text-[var(--color-text-secondary)] transition-colors duration-200 hover:text-[var(--color-accent)] cursor-default select-none">
                {brand}
              </span>
            ))}
          </div>
      ...
      {/* ── NOTRE FLOTTE ─────────────────────────────────────────────────────── */}
      <section className="pt-20 pb-12 bg-[var(--color-background-secondary)]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-12 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-3xl font-black text-[var(--color-text-primary)] sm:text-4xl">Notre catalogue</h2>
              <p className="mt-2 text-sm text-[var(--color-text-secondary)]">Véhicules vérifiés, prêts à être loués ou achetés.</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {(['tous', 'location', 'vente'] as const).map((f) => (
                <button key={f} onClick={() => { setFilter(f); sliderRef.current?.scrollTo({ left: 0, behavior: 'smooth' }); }}
                  className={`rounded-full px-5 py-2 text-xs font-bold capitalize transition-all ${
                    filter === f ? 'bg-[var(--color-accent)] text-white shadow-md' : 'bg-[var(--color-background-primary)] text-[var(--color-text-secondary)] ring-1 ring-[var(--color-border-secondary)] hover:ring-[var(--color-accent)] hover:text-[var(--color-accent)]'
                  }`}>
              ...
              <div className="flex gap-2 ml-1">
                <button onClick={() => slide('left')}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--color-background-primary)] text-[var(--color-text-primary)] shadow-sm ring-1 ring-[var(--color-border-secondary)] transition hover:bg-[var(--color-accent)] hover:text-white hover:ring-[var(--color-accent)]">
          ...
          {voitures.length === 0 ? (
            <div className="flex gap-5 overflow-hidden">
              {[1,2,3,4].map((i) => <div key={i} className="h-80 w-72 shrink-0 animate-pulse rounded-2xl bg-[var(--color-border-tertiary)]" />)}
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex h-48 items-center justify-center rounded-2xl border border-dashed border-[var(--color-border-secondary)] bg-[var(--color-background-primary)] text-sm text-[var(--color-text-secondary)]">
          ...
          <div className="mt-6 flex justify-center">
            <Link href="/catalogue"
              ...
              onMouseEnter={(e) => { const el = e.currentTarget as HTMLElement; el.style.backgroundColor = 'var(--color-accent)'; el.style.color = '#fff'; }}
              onMouseLeave={(e) => { const el = e.currentTarget as HTMLElement; el.style.backgroundColor = ''; el.style.color = 'var(--color-accent)'; }}>
              Voir tout le catalogue
      ...
      {/* ── COMMENT ÇA MARCHE ────────────────────────────────────────────────── */}
      <section className="pt-10 pb-20 bg-[var(--color-background-secondary)]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-14 text-center">
            <h2 className="text-3xl font-black text-[var(--color-text-primary)] sm:text-4xl">Comment ça marche ?</h2>
            <p className="mt-3 text-sm text-[var(--color-text-secondary)] max-w-md mx-auto">Louez ou achetez votre véhicule en 3 étapes simples, depuis votre téléphone ou votre ordinateur.</p>
          </div>

          <div className="grid gap-8 sm:grid-cols-3">
            {HOW_IT_WORKS.map((step, idx) => {
              const stepColor = idx === 0 ? 'var(--color-accent)' : idx === 1 ? 'var(--color-success-text)' : 'var(--color-secondary)';
              return (
                <div key={step.step} className={`reveal group flex flex-col items-center text-center ${idx === 1 ? 'd-200' : idx === 2 ? 'd-300' : 'd-100'}`}>
                  {/* Photo + numéro */}
                  <div className="relative mb-6 w-full overflow-hidden rounded-2xl shadow-lg transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-xl">
                    ...
                    <div className="absolute -right-2 -top-2 flex h-9 w-9 items-center justify-center rounded-full text-sm font-black text-white shadow-md"
                      style={{ backgroundColor: stepColor }}>
                      {idx + 1}
                    </div>
                  </div>
                  <h3 className="mb-2 text-lg font-black text-[var(--color-text-primary)]">{step.title}</h3>
                  <p className="text-sm leading-6 text-[var(--color-text-secondary)] max-w-xs">{step.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* ── NOS SERVICES ─────────────────────────────────────────────────────── */}
      <section id="services" className="pt-0 pb-20 bg-[var(--color-background-secondary)]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-12 text-center">
            <h2 className="text-3xl font-black text-[var(--color-text-primary)] sm:text-4xl">Nos services</h2>
            <p className="mt-3 text-sm text-[var(--color-text-secondary)] max-w-md mx-auto">Location, vente, entretien et assurance tout en un seul endroit.</p>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {SERVICES.map((s, i) => (
              <Link key={s.title} href={s.href}
                className={`reveal-scale group relative overflow-hidden rounded-2xl border border-[var(--color-border-tertiary)] bg-[var(--color-background-primary)] p-6 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:border-transparent block ${['d-100','d-200','d-300','d-400'][i]}`}>
                ...
                <div className="relative">
                  ...
                  <h3 className="mb-2 font-black text-[var(--color-text-primary)]">{s.title}</h3>
                  <p className="text-sm leading-6 text-[var(--color-text-secondary)]">{s.desc}</p>
      ...
      {/* ── TÉMOIGNAGES ──────────────────────────────────────────────────────── */}
      <section className="py-20 bg-[var(--color-background-primary)]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-12 text-center">
            <h2 className="text-3xl font-black text-[var(--color-text-primary)] sm:text-4xl">Avis de nos clients</h2>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {TESTIMONIALS.map((t, i) => (
              <div key={t.name}
                className={`reveal rounded-2xl border border-[var(--color-border-tertiary)] bg-[var(--color-background-secondary)] p-6 transition hover:shadow-md relative overflow-hidden ${['d-100','d-200','d-300'][i]}`}>
                {/* Guillemet décoratif */}
                <div className="absolute right-4 top-4 text-6xl font-black leading-none text-[var(--color-border-tertiary)] select-none">"</div>
                {/* Étoiles */}
                <div className="mb-4 flex gap-0.5">
                  {[1,2,3,4,5].map((s) => <StarIcon key={s} filled={s <= t.stars} />)}
                </div>
                <p className="mb-5 text-sm leading-7 text-[var(--color-text-primary)] relative z-10">"{t.text}"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-[var(--color-text-primary)]">{t.name}</p>
      ...
      {/* ── LOCALISATION ─────────────────────────────────────────────────────── */}
      <section id="localisation" className="py-16 bg-[var(--color-background-secondary)] border-t border-[var(--color-border-tertiary)]">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-10 text-center">
            <h2 className="mt-2 text-3xl font-black text-[var(--color-text-primary)] sm:text-4xl">Nous trouver</h2>
            <p className="mt-3 text-sm text-[var(--color-text-secondary)]">Venez découvrir notre flotte en concession</p>
          </div>
          <div className="grid gap-8 lg:grid-cols-5 items-stretch">
            {/* Carte */}
            <div className="lg:col-span-3 overflow-hidden rounded-2xl border border-[var(--color-border-tertiary)] shadow-sm" style={{ minHeight: 340 }}>
              <iframe
            ...
            <div className="lg:col-span-2 flex flex-col gap-5 justify-center">
              {[
                ...
                <div key={label} className="flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    {lines.map((l, i) => (
                      <p key={i} className={i === 0 ? 'text-sm font-semibold text-[var(--color-text-primary)]' : 'text-xs text-[var(--color-text-secondary)]'}>{l}</p>
                    ))}
                  </div>
                </div>
              ))}
              <a
                ...
                style={{ background: 'var(--color-accent)', width: 'fit-content' }}
              >
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
